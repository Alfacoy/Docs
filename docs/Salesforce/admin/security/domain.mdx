...
* Controlar que hacer cuando un usuario intenta usar la URL de instancia en lugar de la del dominio.
    ...
    * **Redirigir con una advertencia a la misma página dentro del dominio**: no es bloqueanto pero recuerda a los usuarios cúal es la URL que deben usar. Es la opción ideal para reforzar la identidad
    de la marca.
    * **No redirigir (No recomendado)**: es puramente bloqueante y solo se recomienda cuando todos los usuarios esten acostumbrados a utilizar la URL de la marca.
