...
* **Salesforce Authenticator:** Es una aplicación móvil gratuita para de Salesforce que nos permite verificar rápidamente nuestras identidades.
...
* Cuando los usuarios acceden alas API de Salesforce.
* Cuando los usuarios accede a una aplicación, tablero o reporte. Este proceso es llamado **"Autenticación de nivel superior"**.
...
* En el caso de un perfil es lo mismo, pero se configura desde el perfil mismo.
...
### Método para excluir
Puede que tengamos ocaciones en las que debamos excluir a un usuario o bot de MFA. Para hacerlo basta tan solo con armar un conjunto de permisos y habilitar la opción de "Waive Multi-Factor Authentication for Exempt Users" 
...
Si nuestros usuarios utilizan Salesforce Authenticator, hay una gran posibilidad que ellos esten usando la opción de verificar mi identidad automáticamente desde una geolocalización 
segura.
...
En caso de no ser así, lo mejor es desde la página de detales de usuario, presionar en "Desconectar" junto a **Registro de aplicación: Salesforce Authenticator"**.
De está manera, la próxima vez que quiera iniciar sesión, tendrá que conectar nuevamnte su cuenta.
...
nombre de usuario, actividad, activador, método usado, estado, IP, Localicación, etc. Super útil.
...
Se puede configurar flujos de pantallas para ejecutar en los inicios de sesíón.
